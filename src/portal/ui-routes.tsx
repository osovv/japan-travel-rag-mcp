...
function portalStyles(): string {
  ...
  return [
    ...
    `.endpoint-box { background: #f1f5f9; border: 1px solid var(--line); border-radius: 0.5rem; padding: 0.75rem 1rem; font-family: "SFMono-Regular", "Consolas", "Liberation Mono", monospace; font-size: 0.85rem; word-break: break-all; color: var(--accent-hover); margin: 0.5rem 0; }`,
...
export function renderPortalConnectionGuide(mcpEndpointUrl: string): string {
  ...
  return [
    ...
    `<p>Connect your AI agent to the Japan Travel RAG server using the Model Context Protocol (MCP) with OAuth 2.0 authentication.</p>`,
    `</div>`,

    `<div class="section-card">`,
    `<h3>Your MCP Endpoint</h3>`,
    `<div class="endpoint-box">${escapedUrl}</div>`,
    `<p>Authentication: <strong>OAuth 2.0</strong> (automatic via MCP client)</p>`,
    `</div>`,

    `<div class="section-card">`,
    `<h3>Step-by-Step Guide</h3>`,
    `<ol class="steps">`,
    `<li>Open your MCP client configuration (Claude Desktop settings or Claude Code config).</li>`,
    `<li>Add a new MCP server entry with the endpoint URL shown above.</li>`,
    `<li>Set the authentication type to <strong>OAuth 2.0</strong>.</li>`,
    `<li>Save the configuration and restart your MCP client.</li>`,
    `<li>When prompted, authorize via the OAuth flow. Your tester access is already provisioned.</li>`,
    `</ol>`,
    `</div>`,

    `<div class="section-card">`,
    `<h3>Example Configuration (Claude Desktop)</h3>`,
    `<p>Add the following to your Claude Desktop MCP configuration file:</p>`,
    `<div class="code-block">${escapeHtml(exampleConfig)}</div>`,
    `</div>`,

    `<div class="section-card">`,
    `<h3>Example Configuration (Claude Code)</h3>`,
    `<p>Run the following command in your terminal:</p>`,
    `<div class="code-block">claude mcp add japan-travel-rag ${escapedUrl} --transport sse</div>`,
    `</div>`,
  ].join("");
...
function renderSocialButtons(intent: "register" | "login"): string {
  ...
  const githubUrl = escapeHtml(`/portal/auth/start?provider=github&intent=${intent}`);
